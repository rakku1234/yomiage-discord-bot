    ...
    if before.channel is None and after.channel is not None:
        ...
        if dynamic_joins:
            for text_channel_id in dynamic_joins:
                ...
                if text_channel and text_channel.category_id == after.channel.category_id:
...
async def connect_to_voice_channel(guild: discord.Guild, voice_channel: discord.VoiceChannel, text_channel_id: int, debug: bool) -> bool:
    try:
        await voice_channel.connect(self_deaf=True)
        ...
        logger.error(f"{guild.name}の自動参加に失敗しました: {e}")
        return False
