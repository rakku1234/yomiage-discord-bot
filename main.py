...
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    if member.id == client.user.id:
        if before.channel is not None and after.channel is None:
            ...
            if debug:
                logger.debug(f"{member.guild.name}の読み上げチャンネルをデータベースから削除しました")
    ...
    if chat_channel_id:
        ...
        if chat_channel:
            await chat_channel.send(embed=discord.Embed(
                color=discord.Color.dark_blue(),
                description='ボイスチャットからユーザーがいなくなったため退出しました'
            ))
...
async def connect_to_voice_channel(guild: discord.Guild, voice_channel: discord.VoiceChannel, text_channel_id: int, debug: bool) -> None:
    try:
